...
class JoinPage(webapp2.RequestHandler):
  ...
  def post(self, room_id, client_id): 
    #// IRMA client id burada cllient id olusturuyor
    logging.info('----------------------------------')
...
# Irma 
#Baslangic ekranina bir client id aliyorum
class ClientInfo(webapp2.RequestHandler):
  def write_response(self):
      self.response.write(json.dumps({
        'client_id': generate_random(8)
      }))
  def write_call_response(self, client_id):
      self.response.write(json.dumps({
        'calling':client_id
      }))

  def write_any_call_response(self, client_id):
    if client_id not in calling_map:
      calling_map[client_id] = ""
      
    self.response.write(json.dumps({
        'calling_you':calling_map[client_id]
  ...
  def parse_request(self,url):
    ...
    parsedLink = url.split("/")[3:]
    logging.info(parsedLink)
    logging.info("LEN:")
    logging.info(len(parsedLink))
    if len(parsedLink) == 4 and parsedLink[1] == "call":
      logging.info("call")
      logging.info(parsedLink[2])
      calling_map[parsedLink[3]] = parsedLink[2]
      self.write_call_response(parsedLink[3])
    elif len(parsedLink) == 2 and parsedLink[1] != "get_id":
      logging.info("any_call ?")
      logging.info (parsedLink[1])
      self.write_any_call_response(parsedLink[1]);
      if parsedLink[1] in calling_map:
        del calling_map[parsedLink[1]]  
    else:
      logging.info("get_id")
      self.write_response();

  def get(self,*args, **kwargs):
    self.parse_request(self.request.path_url)
...
app = webapp2.WSGIApplication([
    ...
    ('/client/get_id', ClientInfo),
    ('/client/call/([a-zA-Z0-9-_]+)/([a-zA-Z0-9-_]+)', ClientInfo),
    ('/client/([a-zA-Z0-9-_]+)', ClientInfo),
    ('/params', ParamsPage),
