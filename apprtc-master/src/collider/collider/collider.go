...
type Collider struct {
	*roomTable
	dash *dashboard
	registeredClients map[string]io.ReadWriteCloser
}
...
func NewCollider(rs string) *Collider {
	return &Collider{
		...
		dash:      newDashboard(),
		registeredClients : make(map[string]io.ReadWriteCloser), 
	}
}
...
func (c *Collider) Run(p int, useTls bool) {
	...
	if useTls {
		...
		log.Printf("HTTP LISTENING STARTED")
...
func (c *Collider) httpHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("Burda http Handler Geldi")
	log.Printf(html.EscapeString(r.URL.Path))

	if len(p) != 3 {
		c.httpError("Invalid path: "+html.EscapeString(r.URL.Path), w)
		return
	}
	log.Printf("Phttphandler parameters ")
	log.Printf(p[1])
	log.Printf(p[2])
	rid, cid := p[1], p[2]
...
loop:
	for {
		...
		case "register":
			...
			log.Printf("Register Client Id")
			log.Printf(msg.ClientID)
