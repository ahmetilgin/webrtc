...
var AppController = function(loadingParams) {
  ...
  this.myClientID_= $(UI_CONSTANTS.myClientId);
  ...
  Promise.resolve(paramsPromise).then(function(newParams) {
    ...
    if (newParams) {
      Object.keys(newParams).forEach(function(key) {
        ...
        loadingParameters[key] = newParams[key];
      }.bind(this));
    }
    ...
    if (this.loadingParams_.roomId) {
      ...
      if (!RoomSelection.matchRandomRoomPattern(this.loadingParams_.roomId)) {
        ...
        $(UI_CONSTANTS.confirmJoinRoomSpan).textContent = ' "' +
            ...
            console.log('appcontroll input room ='+this.loadingParams_.roomId );
      }
...
//Irma burda baslangıcta  py appcontrollerden alidm 
AppController.prototype.getMyClientID = function() {
      sendAsyncUrlRequest('GET', 'client/get_id').then(function(result) {
        clientId = JSON.parse(result).client_id;
    
        $('#my-client-id').innerText ="My Unique ID: " +  clientId;
...
AppController.prototype.createCall_ = function() {
  ...
  this.infoBox_ = new InfoBox($(UI_CONSTANTS.infoDiv), call_,
  ...
  if (roomErrors && roomErrors.length > 0) {
    ...
    for (var j = 0; j < roomWarnings.length; ++j) {
     ...
     sendAsyncUrlRequest('GET', 'client/call/'+clientId+"/"+document.getElementById("room-id-input").value).then(function(result) {
        console.log(result);
      }).catch(function(error) {
        trace('Initializing; error getting params from server: ' +
            error.message);
  });
 // burda devam etmiyor call_ undefined cunku 
  ...
  call_.onsignalingstatechange =
      this.infoBox_.updateInfoDiv.bind(this.infoBox_);
  call_.oniceconnectionstatechange =
      this.infoBox_.updateInfoDiv.bind(this.infoBox_);
  call_.onnewicecandidate =
      this.infoBox_.recordIceCandidateTypes.bind(this.infoBox_);

  call_.onerror = this.displayError_.bind(this);
  call_.onstatusmessage = this.displayStatus_.bind(this);
  call_.oncallerstarted = this.displaySharingInfo_.bind(this);
...
AppController.prototype.transitionToActive_ = function() {
  ...
  this.infoBox_.setSetupTimes(call_.startTime, connectTime);
  this.infoBox_.updateInfoDiv();
  trace('Call setup time: ' + (connectTime - call_.startTime).toFixed(0) +
      'ms.');
...
AppController.prototype.onKeyPress_ = function(event) {
  switch (String.fromCharCode(event.charCode)) {
    ...
    case 'm':
      if (call_) {
        call_.toggleAudioMute();
        this.muteAudioIconSet_.toggle();
      }
      return false;
    case 'c':
      if (call_) {
        call_.toggleVideoMute();
