...
Call.prototype.joinRoom_ = function() {
  return new Promise(function(resolve, reject) {
    ...
    var path = this.roomServer_ + '/join/' +
        this.params_.roomId+ '/'+ $('#my-client-id').innerText.split(":")[1].trim(" ") + window.location.search;
...
Call.prototype.sendSignalingMessage_ = function(message) {
  ...
  console.log('BURASI JSON MESSAGE'+msgString);
  if (this.params_.isInitiator) {
    ...
    console.log('BURASI sENT SINGALIN MESSAGE' +path);
